...
**任务**：
- ✅ `modules/model`：domain（Model / ModelCredential / RoutingPolicy / QuotaCounter）+ service（19 service tests + 31 domain tests）
- ✅ `libs/vault/crypto/aes_gcm`：AES-GCM encrypt/decrypt + Argon2id KDF（39 unit tests）
- ✅ 0008 migration：7 张表（models / model_credentials / routing_policies / quota_counters / channels / channel_secrets / channel_deliveries）
- ✅ `modules/channel`：domain（Channel / ChannelDelivery）+ service + 19 unit tests（HMAC 签名 / 时间戳 skew / 4 渠道 inbound registry / 失败重试）
- ✅ `LLMPort.model_id`：agent_runtime 接入 model routing（向后兼容 default；42 agent_runtime tests 通过）
- ✅ container 装配 + `/v1/models` + `/v1/model-credentials` + `/v1/routing-policies` HTTP 路由
- ✅ `modules/channel/{inbound, outbound}` 4 渠道适配器（飞书 / 钉钉 / 企微 / Web）+ persistence adapters + AES-GCM webhook cipher + HTTP webhook router（36 channel tests：17 adapter + 19 service）
- ✅ channel_router 注册到 composition main.py；create_app() 启动验证 4 路由（POST /channels / GET /channels / GET /channels/{cid} / PATCH /channels/{cid} / POST /channels/{cid}/webhook / POST /channels/{cid}/send）

**退出标准**：
- ✅ 模型注册 / 路由 / 调用 / 配额闭环
- ✅ 模型凭据加密（AES-GCM）落 DB 明文不存
- ✅ Webhook 签名校验（HMAC-SHA256 + ±300s skew + constant-time compare）
- ✅ 模型路由按租户隔离
- ✅ 渠道 inbound envelope 解析（飞书 / 钉钉 / 企微 / Web 各覆盖：URL verification / 加密占位 / XML legacy / JSON envelope）
- ✅ 渠道接收消息 → 路由到 agent_runtime → 回复（ChannelDispatchSubscriber P7-8 闭环：`ChannelMessageReceived` → 系统 actor turn → `OutboundAdapter.send_reply`）
...
- Plan DSL：JSON / YAML（7 种 kind discriminated union: agent / tool / skill / subplan / parallel / conditional / sequence） ✅
- 单测 + 集成测试 ✅

**退出标准**：
- Plan 执行：串行 / 并行 / 条件分支 ✅
- knowledge 检索 top-k=10，注入上下文 ✅
- Agent 可调用其他 Agent（编排模式） ✅（`SubAgentAdapter` 委托 `RunTurnCompletionUseCase`）
- 知识原文上传到对象存储，DB 存元数据 ✅（`LocalDiskKnowledgeStorage` + `storage_uri`）
...
**P8 完成情况**（2026-09-23，commit 425a508 / 631732e / ab40118）：
- `modules/agent_factory` ✅ domain + 8 SQL/SQL 迁移（0011_agent_factory）/ 12 路由 + composition 装配
- `modules/evaluation` ✅ domain + heuristic scorer + runner + 21 in-memory 测试 + 0012_evaluation 迁移 + SQL repos + 6 路由 + 50-case golden dataset lifespan seed
- `agent_factory ↔ evaluation` 闭环 ✅ `EvaluationServiceAdapter` 实现 `EvaluationQueryPort`，在 lifespan 装配中替换 NoOpEvaluationQuery；POST `/v1/agents/{aid}/versions/{vid}/release` 经 `EvalGate` 校验 `latest_passed_run` + `mean_score >= settings.agent_factory_eval_score_min`
- 跨租户隔离、partial UQ `eval_runs.idempotency_key`、`golden-default` 内置 fixture 全部生效
- 文档：`04-模块设计.md` §4.4.5/§4.4.6 已扩展

---

### P9 · 横切 · observability_module + platform（6 天） ✅ 2026-09-23

**目标**：业务级埋点（运行记录 / 成本 / 质量）；平台设置与计费骨架。

**任务**：
- `modules/observability_module`：domain（RunRecord / CostRecord / PricingCatalog）+ SQL + Subscriber + HTTP + composition wiring
- 成本计算：LLM token × 定价；Tool / Skill 调用成本
- `modules/platform`：domain（Plan / Subscription / TenantSetting）+ SQL + HTTP + 默认 plan seed
- `infra/grafana/` dashboards + `infra/prometheus/` alerts
- subscriber install 接入 lifespan；12 e2e 集成测试

**退出标准**：
- 每次 turn / tool call / skill invoke / memory write 都落 RunRecord + CostRecord ✅
- `/v1/observability/costs` 按 tenant / workspace 聚合 ✅
- 平台级设置读写正常 ✅
- Grafana 多租户 TOP 面板可用 ✅（`eos-overview.json` + `eos-costs.json`）
- 告警规则（按 tenant）部署 ✅（`eos-alerts.yaml`：4 rules）

**实际交付**（commits `d9b2de2` / `7e1e6c3` / `d16558f` / `8cedc91` / `72b4b39` / `32cc35a`）：
- 0013 / 0014 双迁移落地；observability 60+ tests + platform 42 tests + observability 12 e2e tests
- 6 个 settings 字段（pricing JSON + unit costs + currency）；2 个 platform settings
- lifespan 装配：`_ObservabilityFactory` + `_PlatformFactory` + `obs_install(bus, recorder)`
- `infra/grafana/dashboards/` + `infra/prometheus/rules/` 完整
...
| **M6**：model + channel | ✅ 2026-09-22（P6-1..7 全部交付：vault AES-GCM + 0008 迁移 + model domain + model service + 19 service tests + channel domain + channel service + 4 渠道 inbound/outbound adapters + webhook HTTP router + 36 channel tests + container 装配 + main.py 路由注册 + create_app 启动验证） | 多 provider + 飞书 / 钉钉 / 企微 / Web |
| **M7**：orchestration + knowledge | ✅ 2026-09-23（P7-1..8 全部交付：KnowledgeChunkId schema + knowledge domain/SQL/HTTP + orchestration domain/SQL/executor/HTTP + agent_runtime knowledge hook + SubAgentAdapter + RunTurnCompletionUseCase + ChannelDispatchSubscriber；P7-9 docs + verify 已落；单测 + 集成覆盖 sequence / parallel / conditional / agent / tool / skill / subplan + top-k 检索 + webhook 闭环） | 多 Agent + RAG |
| **M8**：agent_factory + evaluation | ✅ 2026-09-23（P8-1..6 全部交付：AgentVersionId 等 5 个 schema + agent_factory domain/SQL/HTTP + evaluation domain/scorer/runner/in-memory 测试 + 0011_agent_factory/0012_evaluation 双迁移 + 50-case golden dataset + EvaluationServiceAdapter 闭环 + lifespan 装配 + docs §4.4.5/§4.4.6 扩展；737 unit tests pass + ruff clean） | 评测门禁生效 |
| **M9**：observability + platform | ✅ 2026-09-23（P9 全部交付：observability domain/PricingCatalog/SQL/Subscriber/HTTP + platform domain/SQL/HTTP/default plans seed + infra/grafana + infra/prometheus + 12 e2e integration tests + docs §4.4.7/§4.4.8） | 全链路埋点 + 平台设置 |
