...
describe('captureAudio', () => {
  ...
  it('returns ok with audio payload on success', async () => {
    ...
    const recorderFactory: MediaRecorderCtor = class extends (recorder as unknown as new (stream: MediaStreamLike, options?: { mimeType?: string }) => MediaRecorderLike) {
  ...
  it('returns oversize when payload exceeds cap', async () => {
    ...
    const captureFactory: MediaRecorderCtor = class extends (bigRecorder as unknown as new (stream: MediaStreamLike, options?: { mimeType?: string }) => MediaRecorderLike) {
      constructor(stream: MediaStreamLike, options?: { mimeType?: string }) {
        super(stream, options);
        recRef.current = this;
      }
