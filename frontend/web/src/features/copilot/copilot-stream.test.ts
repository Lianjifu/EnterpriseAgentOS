...
describe('parseSSEChunk', () => {
  it('parses stage and delta events', () => {
    ...
    const rest = parseSSEChunk(
      ...
      (event, data) => events.push({ event, type: data.type as string }),
    );
    expect(rest).toBe('');
  ...
  it('parses snapshotId on done', () => {
    ...
    parseSSEChunk(
      ...
      (_event, data) => events.push({ type: data.type as string, snapshotId: data.snapshotId }),
    );
  ...
  it('parses multi-segment SSE events', () => {
    ...
    parseSSEChunk(
      ...
      (_event, data) => events.push(data.type as string),
    );
