...
import { buildTurnSummary, parseTurnMeta } from './build-turn-summary';
import { groupTurnPhases } from './group-turn-phases';
import { phaseLabel } from './phase-labels';
import type { TurnPhaseBlock, TurnTaskItem } from './types';
...
export function TurnThoughtPanel({ message, streaming, showNarrative = true }: Props) {
  ...
  const tasks = dedupeTurnTasks((message.turnTasks ?? message.turnMeta?.tasks ?? []) as TurnTaskItem[]);
  const phases = useMemo(
    () => groupTurnPhases(steps, tasks, message.cognitive, streaming, parseTurnMeta(message.turnMeta)),
